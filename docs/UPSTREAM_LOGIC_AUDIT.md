...
- 上游行为：bridge 侧对 >65500 字节的包自动走 `CMD_MULTI` 分片
  ...
  超限也不会断开连接。
- 后果：与已知的 `MULTI` 功能缺口叠加：不但传不了大参数，还会把整个页面
  弄死。协议命令缺失是功能缺口，"以断连回应超限调用" 是错误的失败方式。
- 修复方向：短期改为回发空响应 + 日志（复用上游 “void response 解决 promise”
  的语义，`webui.c:11856-11866`）；长期实现 `MULTI`。
